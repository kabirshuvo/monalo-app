...
import { awardSegmentWordCorrect, getPointsBreakdown } from '@/lib/points/service'
...
export async function POST(request: Request) {
  try {
    ...
    const result = await awardSegmentWordCorrect(session.user.id, word.id, word.word)
    const breakdown = await getPointsBreakdown(session.user.id)
    revalidatePath(SEGMENT_WORD_BASE_PATH)
    ...
    console.error('[POST /api/learning/segment-the-word/celebrate]', error)
