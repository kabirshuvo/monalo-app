...
export default function ThisIsSection({ items, page }: ThisIsSectionProps) {
  ...
  return (
    <section className={`${ecoTheme.card} border-[#fafaf9]/15 p-4 sm:p-5`}>
      <div className="mb-4 flex items-center justify-center gap-2">
        <span className={`${ecoTheme.pill} bg-amber-400/20 text-orange-200`}>Learn</span>
        <h3 className="text-lg font-extrabold text-[#fafaf9] sm:text-xl">This is…</h3>
      </div>
        ...
        {visible.map((item) => {
          ...
          return (
            <button
              ...
              className={`rounded-2xl border-2 overflow-hidden bg-[#fafaf9]/10 text-left transition active:scale-95 ${
                isPlaying
            ...
            >
              <div className={`relative w-full ${ECO_PENGUIN_IMAGE_ASPECT}`}>
                ...
                />
                {isPlaying && (
                  <span className="absolute bottom-1 right-1 rounded-full bg-amber-500 px-1.5 py-0.5 text-[10px] font-bold text-[#fafaf9]">
                    🔊
                  </span>
                )}
              </div>
              <p className="truncate px-2 py-2 text-center text-xs font-bold text-[#fafaf9] sm:text-sm">
                {item.name}
              </p>
            </button>
          )
