...
export default function BalloonLettersPlay({
  ...
  const ask = useCallback(
    (letter: LetterSound) => {
      stopVowelWordsAudio()
      const nextBalloons = spawnBalloons(letters, letter)
    ...
    const tick = (ts: number) => {
      ...
      const next = current.map((balloon) => {
        ...
        if (y < -18) {
          ...
          const replacements = letters.filter((item) => item.id !== targetId)
          const pick =
            Math.random() < 0.45 && targetId
              ? letters.find((item) => item.id === targetId)
      ...
      if (!targetOnScreen && targetId) {
        const letter = letters.find((item) => item.id === targetId)
      ...
      setBalloons(next.map((balloon) => ({
        ...balloon,
        // Display sway without permanently drifting the base x
  ...
  }, [win, checking, letters])

  const choose = (balloon: Balloon) => {
    if (!target || checking || win || balloon.popping) return
    ...
    void api
      .post<{ awarded?: boolean; points?: number }>('/api/learning/balloon-letters/celebrate', {
        letterId: target.id,
      })
      .then((res) => {
        setWin((prev) => {
  ...
  return (
    <div className="space-y-5">
      <section className={`${balloonLettersTheme.card} space-y-3 p-5 text-center sm:p-6`}>
        {win ? (
      ...
      <div className={balloonLettersTheme.playfield} aria-label="Flying balloons">
