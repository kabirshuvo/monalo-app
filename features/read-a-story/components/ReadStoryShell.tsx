...
export default function ReadStoryShell({
  children,
  title,
  ...
  return (
    <div className={readStoryTheme.shell}>
      <EcoPenguinGuide room="read-a-story" />
      <header className={readStoryTheme.header}>
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <div className="flex min-w-0 items-center gap-2">
            <Link href={backHref} className={`${readStoryTheme.btnSecondary} px-3 py-2 text-xs sm:text-sm`}>
              ← Back
            </Link>
            <Link
          ...
          <nav className="flex shrink-0 items-center gap-2 text-sm">
            <Link href={LEARNING_HUB_PATH} className="font-semibold text-amber-300 hover:underline">
      ...
      <footer className={planetFooter}>
        <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-end sm:justify-between sm:px-6">
          <div>
            ...
            <p className="mt-1 text-sm text-[#d6d3d1]">Decodable stories — tap a word to hear it.</p>
          </div>
          <nav className="flex flex-wrap gap-x-4 gap-y-2 text-sm font-semibold text-amber-200">
