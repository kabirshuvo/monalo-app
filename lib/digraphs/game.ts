...
export function paginateItems<T>(items: T[], page: number, perPage = DIGRAPHS_PER_PAGE): T[] {
  const index = Math.max(0, page - 1)
  return items.slice(index * perPage, (index + 1) * perPage)
}

export function totalPages(count: number, perPage = DIGRAPHS_PER_PAGE): number {
