#!/usr/bin/env node
/**
 * Upload public/digraphs/** to Cloudflare R2 (default: monalomedia/digraphs/).
 *
 * Usage:
 *   node scripts/upload-digraphs-r2.mjs
 *   node scripts/upload-digraphs-r2.mjs --force
 */
...
const LOCAL_DIR = path.join(ROOT, 'public', 'digraphs')
const R2_PREFIX = process.env.DIGRAPHS_R2_PREFIX || 'digraphs'
...
async function main() {
  ...
  const bucket = process.env.DIGRAPHS_R2_BUCKET || process.env.R2_BUCKET_NAME || 'monalomedia'
