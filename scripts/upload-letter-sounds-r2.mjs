#!/usr/bin/env node
/**
 * Upload public/letter-sounds/** to Cloudflare R2 (default: monalomedia/letter-sounds/).
 *
 * Usage:
 *   node scripts/upload-letter-sounds-r2.mjs
 *   node scripts/upload-letter-sounds-r2.mjs --force
 */
...
const LOCAL_DIR = path.join(ROOT, 'public', 'letter-sounds')
const R2_PREFIX = process.env.LETTER_SOUNDS_R2_PREFIX || 'letter-sounds'
...
async function main() {
  ...
  const bucket =
    process.env.LETTER_SOUNDS_R2_BUCKET || process.env.R2_BUCKET_NAME || 'monalomedia'
